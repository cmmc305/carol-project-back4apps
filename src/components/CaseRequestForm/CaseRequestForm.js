...
const CaseRequestForm = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    emailAddress: '',
    phoneNumber: '',
    pdfAnalysis: ''         // Campo que pode ser utilizado futuramente (deixa-se vazio)
  });
  ...
  useEffect(() => {
    ...
    const fetchRequest = async () => {
      ...
      try {
        ...
        // Limpa os arrays de novos uploads
        setNewUccFiles([]);
  ...
  const handleSubmit = async (e) => {
    ...
    // Validação dos arquivos (se houver novos uploads)
    if (
    ...
    try {
      ...
      // Se houver resumo de PDF salvo no formData (campo pdfAnalysis), inclui-o
      ...
      // Limpa os arrays de novos uploads e os inputs de arquivo
      setNewUccFiles([]);
      setNewAgreementFiles([]);
      setNewBankStatementsFiles([]);
      setNewSummonsAndComplaintFiles([]);
  ...
  return (
    <Container className={styles.caseRequestContainer}>
      ...
      <Form className={styles.form} onSubmit={handleSubmit}>
        ...
        {/* Renderizar as seções de upload */}
        {renderUploadSections()}

        {/* Upload Agreement (para Request Type diferente de Garnishment) */}
